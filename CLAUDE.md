...
- `supabase/` — **CLI-owned**: `config.toml` and `migrations/`. `migrations/` is the single
  source of truth for schema. Never pass `--workdir` to relocate it — it is a plain `chdir`, so
  the CLI then looks for `supabase/config.toml` *relative* to the new directory. Never create
  `supabase/config.json`; it makes the project visible to some CLI commands and invisible to
  others.
- `db/` — database artifacts the CLI does not own: the Drizzle schema, generated by
  `pnpm db:pull`, and later pgTAP tests. Read `db/README.md` before pointing Drizzle at user
  data — the credential Supabase hands you bypasses row level security two different ways, and
...
Node 24 (see `.nvmrc`), pnpm 10, and Docker for the local Supabase stack.

| Command | What it does |
|---|---|
| `pnpm install && pnpm dev` | Serves the app on `http://localhost:3000` |
| `pnpm supabase start` / `pnpm supabase stop` | Brings the local Supabase stack up and down. Needs Docker |
...
**Never pass `--webpack` to `next build`.** It fails on every `*.module.scss` that reaches for a
common utility — Tailwind's `@property` fallback block is not a pure CSS-Modules selector, and
18 of 36 everyday utilities emit it. Turbopack is the default and tolerates it.
...
- **Tailwind is consumed through `@apply` inside those stylesheets, never inline.** Settled in
  #3; 0.5 (#7) builds the token layer it names.
- **Every `.scss` containing `@apply` opens with `@reference "#globals.css";`** — that exact
  form. It is a package.json `imports` subpath, so it is identical at any directory depth; the
  tsconfig `@/` alias does not work, because Tailwind resolves with enhanced-resolve and reads
  `@/…` as a bare package. Partials included: Sass expands an `@include` before Tailwind sees
  it, so a mixin carrying `@apply` needs the reference to travel with it.
- **`@theme static`, never bare `@theme`, and never nested inside a selector.** Bare `@theme`
  tree-shakes a token out of `:root` when the only consumer is module-side `var()`. A nested
  `@theme` builds with exit 0 and no warning, discards the selector, hoists the variable to
  `:root` and silently overwrites the light value.
